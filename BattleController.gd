...
func _create_random_player(player_role: String) -> MyPlayer:
	...
	# Randomize stats
	player.max_health = 80 + randi() % 41  # 80-120 HP
...
func generate_teams() -> void:
	...
	for p in team1:
		print("Team1:", p.player_name, "(", p.role, ")")
	for p in team2:
		print("Team2:", p.player_name, "(", p.role, ")")
	...
	while true:
		...
		for p in all_players:
			...
			if p.role == "Support":
				...
				if ally != null:
					ally.heal(p.heal_power)
					print("%s heals %s for %d. %s HP: %d" % [p.player_name, ally.player_name, p.heal_power, ally.player_name, ally.current_health])
				else:
					var enemy = choose_enemy_to_attack(p)
					if enemy != null:
						enemy.take_damage(p.attack_power)
						print("%s attacks %s for %d damage. %s HP: %d" % [p.player_name, enemy.player_name, p.attack_power, enemy.player_name, enemy.current_health])
			else:
				var target = choose_enemy_to_attack(p)
				if target != null:
					target.take_damage(p.attack_power)
					print("%s attacks %s for %d damage. %s HP: %d" % [p.player_name, target.player_name, p.attack_power, target.player_name, target.current_health])
